...
export async function deleteAccountRole(
	...
	try {
		...
		if (!accountRole) {
			throw new AccountRoleNotFoundError("Account role not found or already deleted");
		}
		...
		const updateHistoryEntry = {
			...
			changes: {
				...
				"metadata.deletedByTerminal": options.terminalId,
				...(adminAccount && {
		...
		accountRole.metadata.deletedByTerminal = options.terminalId;

		accountRole.metadata.updatedAt = now;
		accountRole.metadata.updatedBy = adminAccount?._id;
		accountRole.metadata.updatedByTerminal = options.terminalId;
